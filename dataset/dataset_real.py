...
def get_data(config, is_train=True):
    if is_train:
        s = np.load(Path(config.real_data_path) / 'high_snr.npy')
        n = np.load(Path(config.real_data_path) / 'noises.npy')
        return s, n
    else:
        s = np.load(Path(config.real_data_path) / 'low_snr.npy')
        return s
...
class TrainDataset(MyDataset):
    def __init__(self, config):
        super(TrainDataset, self).__init__(config, is_train=True)
...
class TestDataset(MyDataset):
    def __init__(self, config):
        super(TestDataset, self).__init__(config,  is_train=False)
